...
feature_matrix = pd.read_csv(
    r"C:\Users\Paula Woest\OneDrive\Desktop\Bachelore Arbeit\Paula\output_data\Session 2_Interaction\final_feature_matrix_session2.csv"
)

# Extract relevant data (modify column names as necessary)
sus = feature_matrix[feature_matrix["sus_res"] == "sus"]["s_in_roi"].values[:10]
res = feature_matrix[feature_matrix["sus_res"] == "res"]["s_in_roi"].values[:9]
# %%
# Create the DataFrame for plotting
data = {
    "s_in_roi": np.concatenate([sus, res]),
...
# Example mouse data (replace with your actual data)
mouse_data_sus = feature_matrix[feature_matrix["sus_res"] == "sus"]["s_in_roi"].values[
    :10
]

mouse_data_res = feature_matrix[feature_matrix["sus_res"] == "res"]["s_in_roi"].values[
    :9
]
...
# Add SEM error bars
plt.errorbar(
    1.2, mean_sus, yerr=sem_sus, fmt="none", ecolor="#1b7837", capsize=5, zorder=0
)
plt.errorbar(
    2.2, mean_res, yerr=sem_res, fmt="white", ecolor="#1b7837", capsize=5, zorder=0
)
...
plt.ylabel("s_in_roi", fontsize=12)
plt.ylim(20, 100)
plt.grid(alpha=0.3)
